...
from app.routes import chat_router, crawl_router, admin_router, analytics_router, conversations_router, triggers_router, handoff_router, platform_router, platform_admin_router, subscriptions_router, byok_router, checkout_router, plans_router
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from app.services.plans import seed_default_plans, backfill_subscription_snapshots
        await seed_default_plans(mongodb)
        migrated_subscriptions = await backfill_subscription_snapshots(mongodb)
        if migrated_subscriptions:
            logger.info(f"Backfilled plan snapshots for {migrated_subscriptions} subscriptions")
        if (
